...
export default function Dashboard({ user }: { user: User }) {
  ...
  const [isBoxOnline, setIsBoxOnline] = useState<boolean>(false);
  const BOX_ID = "01";

  // fetch magazines from firestore
  useEffect(() => {
  ...
  // listen for box online status from realtime DB
  useEffect(() => {
    const statusRef = ref(db, `boxes/${BOX_ID}/status/online`);
    
    const unsubscribe = onValue(statusRef, (snapshot) => {
      const status = snapshot.val();
      setIsBoxOnline(!!status);
    });
